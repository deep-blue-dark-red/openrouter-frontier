...
def add_task_args(parser: argparse.ArgumentParser) -> None:
    ...
    g.add_argument("-o", "--completion-tokens", type=int, default=500, help="Completion tokens per turn, o")
    g.add_argument("--task-tokens", type=int, default=300_000,
                   help="Transcript size the task grows to; N = task_tokens / (a + o)")
    g.add_argument("-N", "--turns", type=int, default=None, help="Explicit number of turns (overrides --task-tokens)")
...
def describe_profile(cfg: ScoringConfig) -> str:
    ...
    parts = [
        f"Task: {cfg.n_turns} turns × ({cfg.new_tokens_per_turn} new + {cfg.completion_tokens} out) → {size} tokens",
